...
/**
 * A kludge to work around the problem that the mongo client tries to access
 * mongodb with an external interface, but the testing mongo only binds to
 * localhost.
 */
public class ValintalaskentaMongodForTestsFactory extends MongodForTestsFactory {
    ...
    public MongoClient newMongo() {
        Field mongoDprocessField = ReflectionUtils.findField(MongodForTestsFactory.class, "mongodProcess");
        mongoDprocessField.setAccessible(true);
        MongodProcess mongodProcess = (MongodProcess) ReflectionUtils.getField(mongoDprocessField, this);
