...
public class SeurantaDaoTest extends AbstractIntegrationTest {
  ...
  public void testOtaSeuraavaLaskentaTyonalleThreadSafety() {
    // luodaan 500 laskentaa yhdellä hakukohteella
    Set<String> expectedHakukohteet = IntStream.range(0, 500).mapToObj(i -> {
      String nimi = "hakukohde" + i;
      this.luoLaskenta("laskenta" + i, List.of(nimi));
      return nimi;
    ...
    CompletableFuture<String>[] hakukohteet = IntStream.range(0, 500)
        .mapToObj(i -> CompletableFuture.supplyAsync(
            () -> this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi" + i, 10), executor)
            .thenApply(h -> h.get().getRight().iterator().next()))
    ...
    // työn alle saadut vastaavat luotuja
