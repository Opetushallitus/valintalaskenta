...
public class SeurantaDaoTest extends AbstractIntegrationTest {
  ...
  public void testLuoLaskentaRoundTrip() {
    // luodaan laskenta
    TunnisteDto tunnisteDto =
        this.seurantaDao.luoLaskenta(
            "userOid",
            "haku1",
            "haun laskenta",
            "hakuOid",
    ...
    Assertions.assertEquals(
        "hakukohdeOid", laskentaDto.getHakukohteet().iterator().next().getHakukohdeOid());
    Assertions.assertEquals(
        "organisaatioOid", laskentaDto.getHakukohteet().iterator().next().getOrganisaatioOid());
  ...
  private LaskentaDto luoLaskenta(
      String nimi, Collection<String> hakukohdeOids, LaskentaTyyppi laskentaTyyppi) {
    return this.seurantaDao
        .haeLaskenta(
            this.seurantaDao
                .luoLaskenta(
                    "userOid",
                    ...
                    hakukohdeOids.stream()
                        .map(oid -> new HakukohdeDto(oid, "organisaatioOid"))
                        .toList())
                .getUuid())
        .get();
  }
  ...
  public void testOtaSeuraavaHakukohdeTyonAlleEiLaskentoja() {
    ...
    Assertions.assertEquals(
        Optional.empty(), this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 10));
  }
  ...
  public void testOtaSeuraavaHakukohdeTyonAllePerusFlow() {
    ...
    ImmutablePair<UUID, Collection<String>> hakukohteet =
        this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 10).get();
    ...
    Collection<String> laskennanHakukohteet =
    ...
    Assertions.assertEquals(
        ...
        this.seurantaDao.haeLaskenta(laskentaDto.getUuid()).get().getTila());
  }
  ...
  public void testSamanLaskennanHakukohteitaTehdaanOidJarjestyksessa() {
    ...
    Assertions.assertEquals(
        "123",
        this.seurantaDao
            .otaSeuraavatHakukohteetTyonAlle("noodi1", 10)
            .get()
            .getRight()
            .iterator()
            .next());
  ...
  public void testEriLaskentojenHakukohteitaTehdaanLuontiJarjestyksessa() {
    ...
    Assertions.assertEquals(
        laskentaDto1.getUuid(),
        this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 10).get().getLeft().toString());
    Assertions.assertEquals(
        laskentaDto2.getUuid(),
        this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 10).get().getLeft().toString());
  }
  ...
  public void testMerkkaaHakukohdeValmiiksi() {
    ...
    this.seurantaDao.merkkaaHakukohteetValmiiksi(
        UUID.fromString(laskentaDto.getUuid()), Collections.singleton("123"));
  ...
  public void testOlemantontaHakukohdettaEiVoiMerkataValmiiksi() {
    ...
    try {
      this.seurantaDao.merkkaaHakukohteetValmiiksi(
          UUID.fromString(laskentaDto.getUuid()), Collections.singleton("678"));
  ...
  public void testMerkkaaHakukohdeEpaonnistuneeksi() {
    ...
    this.seurantaDao.merkkaaHakukohteetEpaonnistuneeksi(
        UUID.fromString(laskentaDto.getUuid()), Collections.singleton("123"), 1, "viesti");
  ...
  public void testEpaonnistunutHakukohdeLasketaanUudestaanKunnesYrityksetTaynna() {
    ...
    this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 10);
    this.seurantaDao.merkkaaHakukohteetEpaonnistuneeksi(
        UUID.fromString(laskentaDto.getUuid()), List.of("123"), 3, "Kolossaalinen epäonnistuminen");
    this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 10);
    this.seurantaDao.merkkaaHakukohteetEpaonnistuneeksi(
        UUID.fromString(laskentaDto.getUuid()), List.of("123"), 3, "Kolossaalinen epäonnistuminen");
    this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 10);
  ...
  public void testSamanNoodinHakukohteidenMaaraRajoitettu() {
    ...
    Assertions.assertTrue(
        this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 1).isPresent());
  ...
  public void testEriNoodienHakukohteidenMaaraItsenainen() {
    ...
    Assertions.assertTrue(
        this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 1).isPresent());
    Assertions.assertTrue(
        this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi2", 1).isPresent());
  }
  ...
  public void testValmistaLaskentaaEiVoiPeruuttaa() {
    // luodaan laskenta yhdella hakukohteella ja merkataan hakukohde valmiiksi jolloin myös laskenta
    // on valmis
    LaskentaDto laskentaDto = this.luoLaskenta("laskenta", List.of("123"));
    this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 10);
    this.seurantaDao.merkkaaHakukohteetValmiiksi(
        UUID.fromString(laskentaDto.getUuid()), List.of("123"));
  ...
  public void testLaskeSamaHakukohdeMonesti() {
    ...
    this.seurantaDao.merkkaaHakukohteetValmiiksi(
        UUID.fromString(laskentaDto.getUuid()), hakukohteet);
  ...
  public void testValintaRyhmalaskentaPerusFlow() {
    LaskentaDto laskentaDto =
        this.luoLaskenta("laskenta", List.of("123", "234", "345"), LaskentaTyyppi.VALINTARYHMA);

    Optional<ImmutablePair<UUID, Collection<String>>> tyonAlla =
        this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi1", 1);
    Collection<String> hakukohteet = tyonAlla.get().right;

    // kaikki hakukohteet otetaan työn alle yhtäaikaa, maxYhtaaikaisetHakukohteet-parametrista ei
    // välitettä
  ...
  public void testOtaSeuraavaLaskentaTyonalleThreadSafety() {
    ...
    Set<String> expectedHakukohteet =
        IntStream.range(0, 500).mapToObj(i -> "hakukohde" + i).collect(Collectors.toSet());
    ...
    CompletableFuture<String>[] hakukohteet =
        IntStream.range(0, 500)
            .mapToObj(
                i ->
                    CompletableFuture.supplyAsync(
                        () -> {
                          Optional<ImmutablePair<UUID, Collection<String>>> result =
                              this.seurantaDao.otaSeuraavatHakukohteetTyonAlle("noodi" + i, 10);
    ...
    Set<String> actualHakukohteet =
        Arrays.stream(hakukohteet).map(f -> f.join()).collect(Collectors.toSet());
  ...
  public void testMerkitseHakukohdeLasketuksiThreadSafety() {
    ...
    Set<ImmutablePair<UUID, String>> expectedHakukohteet =
        IntStream.range(0, 500)
            .mapToObj(
                i -> {
                  Collection<String> hakukohteet = List.of(i + "_1", i + "_2");
                  UUID uuid =
                      UUID.fromString(this.luoLaskenta("laskenta" + i, hakukohteet).getUuid());
                  Set<ImmutablePair<UUID, String>> hks =
                      hakukohteet.stream()
                          .map(hk -> new ImmutablePair<>(uuid, hk))
                          .collect(Collectors.toSet());
                  return hks;
                })
    ...
    expectedHakukohteet.parallelStream()
        .forEach(
            hk -> {
              Assertions.assertEquals(
                  LaskentaTila.VALMIS,
                  this.seurantaDao.haeYhteenveto(hk.getLeft().toString()).get().getTila());
            });
  }
}
