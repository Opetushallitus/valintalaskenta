...
public interface TulosValintakoeOsallistuminenRepository
  ...
  @Query(
      "select h_oid from ("
          + "select hakemus_oid h_oid from valintakoe_osallistuminen where created_at between :start and :end "
          + " union "
          + "select vo.hakemus_oid h_oid from valintakoe_osallistuminen vo "
          + "join hakutoive h on h.valintakoe_osallistuminen = vo.id where h.created_at between :start and :end "
          + " union "
          + "select vo.hakemus_oid h_oid from valintakoe_osallistuminen vo "
          + "join hakutoive h on h.valintakoe_osallistuminen =  vo.id "
          + "join valintakoe_valinnanvaihe vvv on vvv.hakutoive = h.id where vvv.last_modified between :start and :end "
          + " union "
          + "select vo.hakemus_oid h_oid from valintakoe_osallistuminen vo "
          + "join hakutoive h on h.valintakoe_osallistuminen =  vo.id "
          + "join valintakoe_valinnanvaihe vvv on vvv.hakutoive = h.id "
          + "join valintakoe vk on vk.valintakoe_valinnanvaihe = vvv.id where vk.last_modified between :start and :end"
          + ") hakemus")
